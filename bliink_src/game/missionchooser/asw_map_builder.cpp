...
static ConVar asw_vbsp2( "asw_vbsp2", "0", FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY ); // 0 = Use default map builder (VBSP.EXE), 1 = Use new, experimental level builder (VBSP2LIB.LIB)
static ConVar tilegen_retry_count( "tilegen_retry_count", "20", FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY, "The number of level generation retries to attempt after which tilegen will give up." );
ConVar asw_regular_floor_texture( "asw_regular_floor_texture", "REGULAR_FLOOR", FCVAR_NONE | FCVAR_DEVELOPMENTONLY, "Regular floor texture to replace" );
ConVar asw_alien_floor_texture( "asw_alien_floor_texture", "ALIEN_FLOOR", FCVAR_NONE | FCVAR_DEVELOPMENTONLY, "Alien floor texture used for replacement" );
